...
.PHONY: build build-core build-airflow build-api-ml test download-data apply delete forward forward-kill warehouse batch \
...
reset-data:
	@echo "[1/6] Dừng producer/consumer đang ghi dữ liệu..."
	...
	@echo "[2/6] Xóa sạch ClickHouse batch tables..."
	...
	@echo "[3/6] Xóa raw data, batch views và checkpoints trong MinIO..."
	...
	@echo "[4/6] Xóa Elasticsearch indices và Redis cache..."
	...
	@echo "[5/6] Restart Kafka và tạo lại topics..."
	...
	@echo "[5/6] Xóa lịch sử Airflow (best effort)..."
	$(KUBECTL) exec -n $(NAMESPACE) deployments/airflow-scheduler -- \
	  airflow db clean \
	    ...
	    --skip-archive \
	    --yes || true
	@echo "[6/6] Khôi phục speed-streaming và object-store-writer..."
